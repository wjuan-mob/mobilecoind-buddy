use crate::{Config, Worker, TokenInfo};
use egui::{Button, TopBottomPanel, CentralPanel, Layout, Align};
...
impl App {
    ...
    pub fn new(cc: &eframe::CreationContext<'_>, _config: Config, worker: Worker) -> Self {
            ...
            match self.mode {
                ...
                Mode::Send => {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        let scaled_value_str = self.send_value.entry(self.send_token_id).or_default();
                        ...
                        let okay_to_submit: Result<u64, String> = current_token_info.ok_or("must select a token".to_string()).and_then(|info: &TokenInfo| -> Result<u64, String> {
                            ...
                            let rescaled_value = parsed_value.checked_mul(scale).ok_or("decimal overflow".to_string())?;
